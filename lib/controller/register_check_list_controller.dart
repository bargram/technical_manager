...
class RegisterCheckListController extends GetxController {
  ...
  static Future run({
    ...
    try {
      ...
      String serialNumber = await deviceInfoPlugin.androidInfo
          .then((value) => value.serialNumber);
      GeneralServiceResult result = await CheckListService.register(
        ...
        salamat_sistem_satmayeshi_garmayeshi:
            salamat_sistem_satmayeshi_garmayeshi,
        ...
        mosalas_shabrang_va_cheraq_cheshmak_zan:
            mosalas_shabrang_va_cheraq_cheshmak_zan,
        ...
        amade_be_kar_bodan_sanjesh_sorat_zaman:
            amade_be_kar_bodan_sanjesh_sorat_zaman,
      ...
      if (result.tokenExp) {
        ...
        Navigator.of(context).pushAndRemoveUntil(
            MaterialPageRoute(builder: (_) => LoginOtpPage()),
            (route) => false);
        return;
      }

      if (!result.connection) {
        //handle connection problem event
        setLoadingState(false);
        await alert(
            context: context,
            title: '',
      ...
      if (result.success) {
        ...
        setLoadingState(false);
        await alert(
            context: context, title: '', message: result.message.toString());
